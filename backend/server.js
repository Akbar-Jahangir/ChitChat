  ...
  cors({
    origin: function (origin, callback) {
      const allowedOrigins = [
        process.env.CLIENT_URL || "https://chit-chat-dusky-ten.vercel.app",
        "http://localhost:3000",
        "http://localhost:5173",
      ];
...
app.post("/join-presence", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `User ${userId} is now online. Total online users: ${onlineUsers.size}`
    );
    res.status(200).json({
      success: true,
...
app.post("/leave-presence", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `User ${userId} is now offline. Total online users: ${onlineUsers.size}`
    );
    res.status(200).json({
      success: true,
      onlineCount: onlineUsers.size,
